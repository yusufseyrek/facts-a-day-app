...
function NativeAdCardComponent({
  ...
  // Inline list usage and carousel usage handle outer spacing themselves.
  const needsMargin = !nativeAdProp && !cardWidth;

  // No ad bound yet (pool still loading, no-fill, premium, etc.) — render
  // nothing. Lists that wired `onAdFailed` drop the cell on terminal failure;
  // transient 'loading' windows are effectively invisible.
  if (!nativeAd) {
    return null;
  }
