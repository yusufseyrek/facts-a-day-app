...
/** @deprecated Use addCategoryKeyword instead */
export function addKeyword(keyword: string): void {
  addCategoryKeyword(keyword);
}

/** @deprecated Not needed with single category approach */
export function addKeywords(keywords: string[]): void {
  // Only use the last keyword (most recent)
  if (keywords.length > 0) {
    addCategoryKeyword(keywords[keywords.length - 1]);
  }
}

/** @deprecated Use clearCategoryKeyword instead */
export function clearDynamicKeywords(): void {
  clearCategoryKeyword();
}

/** @deprecated Use getCategoryKeyword instead */
export function getDynamicKeywords(): string[] {
  return currentCategoryKeyword ? [currentCategoryKeyword] : [];
}

/** @deprecated Use getCoreKeywords instead */
export function getBaseKeywords(): string[] {
  return getCoreKeywords();
}

/** @deprecated Not needed with simplified approach */
export function setUserCategories(_categories: string[]): void {
  // No-op: We only track the category from the last viewed fact now
}
